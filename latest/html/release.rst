...
If ``bundle_env2.6.tgz`` and ``bundle_env2.7.tgz`` exist run the commands:
.. code:: bash
...
Once your 2.7 virtual environment is built, run the same move commands and change 2.6 to 2.7. 

These steps will configure your virtual environments to be able to build bundles.
...
.. code:: bash
    ...
    source /path/to/home/bundle_env2.6/bin/activate
    cd /path/to/home/bundle_env2.6/src
    rm -rf pycbc
...
.. note::
   ...
   The update will need to be done for the 2.6 and 2.7 virtual environments.
...
In vim run the command after pressing ``shift+;``:
